...
contract Withdraw is Bridge {
    ...
    function receiveSuccinct(address srcAddress, bytes calldata callData) public {
        require(msg.sender == foreignAmb, "Only foreign amb can call this function");
        require(
            srcAddress == homeDeposit,
            "Only home deposit can trigger a message call to this contract."
        );
        (address recipient, uint256 amount, address tokenAddress) =
            abi.decode(callData, (address, uint256, address));
